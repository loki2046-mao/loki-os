...
(() => {
 ...
 function dress(){
  ...
  if(mast){
   ...
   mast.insertAdjacentHTML('beforeend',`<figure class="masthead-object"><img src="../assets/${isWorks?'loki-ticket-welcome.webp':'loki-method-volumes.webp'}" alt="${isWorks?'Loki 举起票根，欢迎翻看作品':'五本彩色制作手册与夹在其中的书签'}"><figcaption>${isWorks?'把想法，做成看得见的作品。':'常用的方法，装订成册。'}</figcaption></figure>`);
  ...
  if(selected&&panel&&!panel.querySelector('.perspective-story')){
   ...
   if(parts[1]==='works'&&selected.systemEvidence?.length){
    section.innerHTML='<div class="making-notes-title"><span>THE MAKING OF</span><h3>这件作品，怎么做的。</h3></div><div class="making-evidence">'+selected.systemEvidence.map(e=>'<article><h4>'+esc(e.label)+'</h4><p>'+esc(e.value)+'</p></article>').join('')+'</div>';
   }else if(flow){
    section.innerHTML='<div class="making-notes-title"><span>METHOD / 工作路径</span><h3>从哪里开始，带走什么。</h3></div><ol class="method-path">'+flow.map((v,i)=>'<li><span>0'+(i+1)+' / '+['输入','判断与处理','带走'][i]+'</span><b>'+esc(v)+'</b></li>').join('')+'</ol>'+(profile?'<div class="making-evidence"><article><h4>什么时候会用</h4><p>'+esc(profile.trigger)+'</p></article><article><h4>我会把关什么</h4><p>'+esc(profile.judgment)+'</p></article></div>':'');
  ...
  if(!document.querySelector('.publication-footer')){
   document.querySelector('#app').insertAdjacentHTML('beforeend',`<footer class="publication-footer"><span class="footer-signature">LOKI</span><div><b>散场以后，还有想做的事。</b><p>作品、方法，以及下一次相遇。</p></div><a href="../index.html#break">中场歇一会儿 ↗</a><a href="../index.html#about">认识 Loki ↗</a></footer>`);
  }
 }
