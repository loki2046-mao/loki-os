...
const roles=[
{id:'writer',title:'AI 内容创作',eyebrow:'撰稿人卡',icon:'Aa',color:'pink',body:`<span class="object-kicker">撰稿人 / 赛博小熊猫 Loki</span><h3>我写的东西，<br>最后都会做出来。</h3><p>我在公众号【赛博小熊猫 Loki】写 AI，也把写作里反复遇到的问题做成方法。</p><div class="role-routes"><a href="#skills-writing">翻开写作方法 <span>6 个条目 →</span></a><button data-wechat>看看公众号 <span>展开二维码 ⤢</span></button></div><small>这里接回公众号本身，不跳转旧内容站。</small>`},
{id:'coder',title:'Vibe coder',eyebrow:'制作通行证',icon:'↗',color:'blue',body:`<span class="object-kicker">制作中 / 想法需要一个落点</span><h3>想要什么，<br>就把它做成能用的。</h3><p>做 AI 产品、写小工具、把常用方法留成 Skill。作品节目单里，可以看到实际界面和当前进展。</p><div class="pass-strip">LOKI <span>想法 → 制作 → 检查</span></div><div class="role-routes"><a href="#works">看十一件作品 <span>翻到节目单 →</span></a><a href="https://github.com/loki2046-mao" target="_blank" rel="noopener">Loki 的 GitHub <span>个人主页 ↗</span></a><a href="#skills-building">构建方法 <span>3 个条目 →</span></a></div>`},
...
const qr=document.createElement('dialog');qr.id='wechat-view';qr.innerHTML=`<button class="return" id="close-wechat">收起联系卡 ×</button><h2>找到 Loki</h2><p class="contact-lead">几个能稳定找到我的入口，扫码或写信都行。</p><div class="contact-grid"><div class="contact-card"><img src="assets/loki-wechat-official.jpg" alt="赛博小熊猫 Loki 公众号二维码"><b>公众号</b><small>赛博小熊猫 Loki，微信扫一扫</small><a href="assets/loki-wechat-official.jpg" download="赛博小熊猫Loki-公众号.jpg">保存 ↓</a></div><div class="contact-card"><img src="assets/contact/qr-personal-wechat.webp" alt="Loki 个人微信名片二维码"><b>个人微信</b><small>加好友时说下怎么认识的</small><a href="assets/contact/qr-personal-wechat.webp" download="Loki-个人微信.webp">保存 ↓</a></div><div class="contact-card"><img src="assets/contact/qr-xiaohongshu.webp" alt="赛博小熊猫 Loki 的小红书主页二维码"><b>小红书</b><small>赛博小熊猫 Loki，也在更新</small><a href="assets/contact/qr-xiaohongshu.webp" download="Loki-小红书.webp">保存 ↓</a></div></div><div class="contact-row"><span>GitHub</span><a href="https://github.com/loki2046-mao" target="_blank" rel="noopener">github.com/loki2046-mao ↗</a></div><div class="contact-row"><span>邮箱</span><span class="contact-mail"><a href="mailto:lokimao2046@gmail.com">lokimao2046@gmail.com</a><button type="button" class="copy-mail" data-copy="lokimao2046@gmail.com">复制</button></span></div>`;document.body.append(qr);$('#close-wechat').onclick=()=>qr.close();qr.onclick=e=>{if(e.target===qr)qr.close()};qr.addEventListener('click',e=>{const b=e.target.closest('.copy-mail');if(!b)return;const mail=b.dataset.copy;const done=()=>{b.textContent='已复制 ✓';setTimeout(()=>{b.textContent='复制'},1600)};if(navigator.clipboard?.writeText){navigator.clipboard.writeText(mail).then(done)}else{const t=document.createElement('textarea');t.value=mail;document.body.append(t);t.select();document.execCommand('copy');t.remove();done()}});document.addEventListener('click',e=>{if(e.target.closest('[data-wechat]'))qr.showModal()});
