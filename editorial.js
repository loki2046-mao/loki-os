...
const kinds={visual:['VISUAL LAB','视觉试印室','我出的图和海报，大多收在这里。',6,'#dffa4a'],writing:['WRITER’S ROOM','文字排练室','写东西反复遇到的事，我记成了方法。',1,'#ffa6ca'],knowledge:['READING FILE','阅读资料室','读完，我会留下自己的判断。',13,'#becbff'],building:['BACKSTAGE TOOLS','后台工作台','想用的工具，我自己做一个。',5,'#69dcea'],personal:['PERSONAL COLLECTION','私人收藏册','私人的喜欢，我也单独收了一册。',4,'#ffac73'],works:['ON THE WORKBENCH','作品制作档案','成品后面，大多还留着制作记录。',8,'#72dfde']};
...
function dress(){
 ...
 const notes=panel.querySelector('.making-notes');if(notes){notes.insertAdjacentHTML('beforeend','<p class="pencil-note">↳ 这里的记录，都来自我真实做过的东西。</p>');}
 const details=panel.querySelector(':scope>details');if(details)details.classList.add('file-envelope');
 const nav=panel.querySelector('.item-navigation');const ticket=document.createElement('aside');ticket.className='collectible-ticket';ticket.innerHTML=`<div><small>LOKI / TAKE THIS IDEA</small><b>${item.title}</b><span>留着，下次真的会用上。</span></div><button type="button" aria-pressed="false"></button><i aria-hidden="true"></i>`;nav?.before(ticket);bindSave(ticket.querySelector('button'),item);
 ...
 const about=document.querySelector('#about-page');if(about&&!about.querySelector('.personal-pass')){about.insertAdjacentHTML('beforeend',`<aside class="personal-pass"><div>${sticker(6)}</div><section><span class="editorial-kicker">AUDIENCE ↔ CREATOR</span><h3>这一册里，<br>有喜欢，也有做过的事。</h3><p>这里放的是我做过的东西：演出、图、字，还有几个自己用的小工具。</p><a href="detail-study/index.html#catalogue/personal">翻开私人收藏册 ↗</a></section></aside>`)}
 const rest=document.querySelector('#break-page');if(rest&&!rest.querySelector('.intermission-collage')){rest.insertAdjacentHTML('afterbegin',`<div class="intermission-collage"><span class="rest-sign">暂时离开工作台<br><b>INTERMISSION</b></span>${sticker(14)}<span class="rest-note">先歇一会儿，<br>不急着继续往下翻。</span></div>`);rest.insertAdjacentHTML('beforeend','<div class="ticket-wallet"><h3>我的随身票夹</h3><p>你在内页夹的票根，都收在这里。</p><div class="wallet-items"></div></div>')}
